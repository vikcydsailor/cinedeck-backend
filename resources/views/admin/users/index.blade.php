                ...
                <table class="table">
                    ...
                    @foreach($users as $key => $user)
                        <tr data-entry-id="{{ $user->id }}">
                            ...
                            <td>
                                @if (auth()->user()->hasRole('Admin'))
                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="badge bg-danger">Edit</a>
                                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="badge bg-info" onclick="return confirm('Are you sure you want to delete this user?')">Delete</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
